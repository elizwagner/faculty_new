...
nontenure_data_subgroup_long <- nontenure_data_subgroup %>%
  ...
  mutate(
    Group = recode(Group,
      scientist = "Scientist",
      respol    = "Research or Practice Associate",
      rpt       = "RPT Professorial",
      other     = "Other"
    ), 
    Group = factor(Group, levels = c("Scientist", "Research or Practice Associate", "RPT Professorial", "Other"))
    )

# 4) Colors (unchanged)
group_colors <- c(
  "Scientist"                   = "orchid4",
  "Research or Practice Associate"= "springgreen3",
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Load data
department_questions <- read_excel("department_summary.xlsx")
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Reshape to long format including department counts and proportions
department_questions_long <- department_questions %>%
  select(
    Question, Category, Label.y, Level,
    starts_with("Count_"),
    starts_with("Proportion_all")
  ) %>%
  pivot_longer(
    cols = starts_with("Count_") | starts_with("Proportion_all"),
    names_to = c(".value", "Group"),
    names_pattern = "(Count|Proportion_all)_(.*)"
  ) %>%
  mutate(
    Group = recode(Group,
      "MH"                   = "Mental Health",
      ...
      "HBS"                  = "Health, Behavior & Society",
      "EHE"                  = "Environmental Health & Engineering",
      "IH"                   = "International Health",
      "PFRH"                 = "Population, Family & Reproductive Health",
      "Biostat"              = "Biostatistics",
      "BMB"                  = "Biochemistry & Molecular Biology",
      "MMI"                  = "Molecular Microbiology & Immunology"
    ),
    Group = factor(Group, levels = c(
      "Mental Health", "Health Policy & Management", "Epidemiology", "Health, Behavior & Society",
      "Environmental Health & Engineering", "International Health", "Population, Family & Reproductive Health",
      "Biostatistics", "Biochemistry & Molecular Biology", "Molecular Microbiology & Immunology", "Prefer Not to Say"
    ))
  )

# Define department colors (optional - customize as needed)
department_colors <- c(
  "Mental Health"                       = "cornflowerblue",
  "Health Policy & Management"         = "#bfd3e6",
  "Epidemiology"                       = "#9ebcda",
  "Health, Behavior & Society"         = "#8c96c6",
  "Environmental Health & Engineering" = "#8c6bb1",
  "International Health"               = "#88419d",
  "Population, Family & Reproductive Health" = "#810f7c",
  "Biostatistics"                      = "#4d004b",
  "Biochemistry & Molecular Biology"   = "#3f007d",
  "Molecular Microbiology & Immunology" = "slateblue2",
  "Prefer Not to Say"                  = "#2a004d"
)

department_questions_long <- department_questions_long %>%
filter(!is.na(Group)) 
```
...
```{r, fig.height = 20, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}

demographics_title_data <- department_questions_long %>%
  filter(
    Category == "Demographics",
    Level != "NA",
    Question != "_v195",
...
# Filter for Rank/Title category, exclude Level == "NA", exclude 'Other - Text', and include only selected questions
rank_title_data <- department_questions_long %>%
  filter(
    Category == "Rank/Title",
    Level != "NA",
    !str_detect(Label.y, "Other - Text"),
...
selected_questions <- c("_v10", "_v11")


retitiling_data <- department_questions_long %>%
  filter(
    Category == "Retitling Process",
    Level != "NA",
    !str_detect(Label.y, "Other - Text"),
...
```{r, fig.height = 20, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}
service_data <- department_questions_long %>%
  filter(Category == "Service", 
             Level != "NA",
             Question != "_v31",
                  Question != "_v43",
...
# Filter for Rank/Title category and exclude rows where Level == "NA"
prof_dev_data <- department_questions_long %>%
  filter(Category == "Professional development and promotion", 
             Level != "NA",
          Question != "_v61",
    !str_detect(Label.y, "Other - Text")
...
```{r, fig.height = 27, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, fig.height = 20, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}
# Filter for Rank/Title category, exclude Level == "NA", exclude 'Other - Text', and include only selected questions
well_being_data <- department_questions_long %>%
  filter(Category == "Well-being", 
             Level != "NA",
             Question != "_v81",
    !str_detect(Label.y, "Other - Text")
...
```{r, fig.height = 20, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}
# Filter for Rank/Title category and exclude rows where Level == "NA"
equity_data <- department_questions_long %>%
  filter(Category == "Equity, morale, respect, and satisfaction", 
             Level != "NA",
         Question != "_v92",
    !str_detect(Label.y, "Other - Text")
...
```{r, fig.height = 20, fig.width = 7, echo=FALSE, warning=FALSE, message=FALSE}
# Filter for Rank/Title category and exclude rows where Level == "NA"
effort_data <- department_questions_long %>%
  filter(Category == "Effort Distribution", 
             Level != "NA",
    !str_detect(Label.y, "Other - Text")
  )
